...
echo "set term postscript eps size 23,15 color solid font 'Helvetica,43'" >> $plot
...
echo "set rmarg 30" >> $plot
#echo "set xlabel \"time (s)\" font \"Helvetica,48\"" >> $plot
#echo "set xtics out nomirror rotate font \"Helvetica,30\"" >> $plot
echo "set key center right outside nobox" >> $plot

echo "set ytics font \"Helvetica,36\"" >> $plot
echo "set y2tics font \"Helvetica,36\"" >> $plot
...
echo "plot '$data' using 1:6 with lines lw 3 title \"rchar(bytes/sec)\", '$data' using 1:7 with lines lw 3 lt 2 title \"wchar(bytes/sec)\"" >> $plot
...
echo "set xtics out nomirror rotate font \"Helvetica,30\"" >> $plot
echo "set xlabel \"time (s)\" font \"Helvetica,52\" offset 0,1" >> $plot
echo "plot '$data' using 1:2 with lines lw 3 title \"# threads\" axes x1y1, '$data' using 1:3 with lines lw 3 lt 2 title \"% CPU\" axes x1y2" >> $plot
